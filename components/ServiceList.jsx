import {
  ...
  function ServiceList() {
    ...
    const moreActions = [
      ...
      { icon: <Icon source={IconsIcon} color="base" />, label: 'All service reviews', onClick: () => alert('All service reviews') },
      { icon: <Icon source={IconsIcon} color="base" />, label: 'All service reviews to curate', onClick: () => alert('All service reviews to curate') },
      { icon: <Icon source={IconsIcon} color="base" />, label: 'All service reviews', onClick: () => alert('All service reviews') },
    ];
    ...
    useEffect(() => {
      ...
      setLoading(true);
      fetch(`https://aniyor-backend.onrender.com/services?vendorUid=${vendorUid}`)
        .then((res) => {
          if (!res.ok) {
    ...
    const sortOptions = [
      { field: 'title', label: 'Service name', directions: ['A-Z', 'Z-A'] },
    ...
    } = useIndexResourceState(services);
    ...
    const handleTabChange = (selectedTab) => {
      ...
      if (selectedTab === 'All') {
        ...
        const filtered = services.filter(service => 
          service.status?.toLowerCase() === selectedTab.toLowerCase()
        );
    ...
    const handleSearch = (query) => {
      ...
      if (query.trim() === '') {
        ...
        if (tab === 'All') {
          ...
          const filtered = services.filter(service => 
            service.status?.toLowerCase() === tab.toLowerCase()
          );
          setServices(filtered);
        }
      } else {
        ...
        const filtered = services.filter(service => {
          const matchesSearch = 
            ...
            (service.vendor && service.vendor.toLowerCase().includes(query.toLowerCase())) ||
            (service.productType && service.productType.toLowerCase().includes(query.toLowerCase()));
          
          const matchesTab = tab === 'All' || service.status?.toLowerCase() === tab.toLowerCase();
    ...
    const rowMarkup = services.map((service, index) => {
      const inventoryValue = service.totalInventory || 0;
      ...
      return (
        <IndexTable.Row
          id={service.id}
          key={service.id}
          selected={selectedResources.includes(service.id)}
          position={index}
        >
          <IndexTable.Cell>
            {service.image || service.images ? (
              <img
                src={service.image || (Array.isArray(service.images) ? service.images[0] : service.images)}
                alt={service.title}
          ...
          <IndexTable.Cell>
            <Text variant="bodyMd" fontWeight="semibold">{service.title}</Text>
          </IndexTable.Cell>
          <IndexTable.Cell>
          ...
          <IndexTable.Cell>{service.salesChannels ?? 5}</IndexTable.Cell>
          <IndexTable.Cell>{service.markets ?? 2}</IndexTable.Cell>
          <IndexTable.Cell>{service.productCategory ?? '—'}</IndexTable.Cell>
          <IndexTable.Cell>{service.productType || '—'}</IndexTable.Cell>
          <IndexTable.Cell>{service.vendor || '—'}</IndexTable.Cell>
        </IndexTable.Row>
      );
    });
    ...
    const sortFields = [
      ...
      { field: 'productType', label: 'Service type' },
      { field: 'publishingError', label: 'Publishing error' },
    ...
    if (loading) {
      return (
        <Box padding="400" minHeight="200px" display="flex" alignItems="center" justifyContent="center">
          <Spinner accessibilityLabel="Loading services" size="large" />
        </Box>
      );
    }
        ...
        <ContentWrapper>
          <HeadingRow>
            ...
            <ActionsWrapper>
              ...
              <PrimaryButton onClick={handleAddService}>Add Service</PrimaryButton>
            </ActionsWrapper>
          </HeadingRow>
          <TitleSummaryGap />
          <CsvImportModal 
          ...
          <SummaryCardsRow>
            <SummaryCard>
              <Text variant="bodyMd" color="subdued">Services by sell-through rate</Text>
              <Text variant="headingMd">8.2% · 31%</Text>
            </SummaryCard>
            <SummaryCard>
              <Text variant="bodyMd" color="subdued">Services by days of inventory remaining</Text>
              <Text variant="headingMd">31.9+ days</Text>
            </SummaryCard>
            <SummaryCard>
              <Text variant="bodyMd" color="subdued">ABC service analysis</Text>
